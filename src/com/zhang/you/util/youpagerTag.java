 ...
 * 
 */
public class youpagerTag extends TagSupport {
	...
	private String getRender() {
		...
		for (int i = 0; i < pager.getTotalPages(); i++) {
			if ((i + 1) == pager.getCurpageindex())
				sb.append(MessageFormat.format("<a class=\"{1} youpager-pagebutton\" pageindex=\"{0}\" >{0}</a>\r\n", i+1,
						getCurrentpageclass()));
			else
				sb.append(MessageFormat.format("<a pageindex=\"{0}\" class=\"youpager-pagebutton\">{0}</a>\r\n", i + 1));
		}
		//sb.append("<input type=\"hidden\" id=\"currentpageindex\" name=\"currentpageindex\"/>\r\n");
		
		sb.append("<input id=\"youpager_txtgopage\" type=\"text\" style=\"width: 30px\" value=\"1\">\r\n");
		
		sb.append("<a  class=\"next youpager-btngopage\" href=\"#\" >跳转到</a>\r\n");
	...
	private String eventHandler(){
		...
		sb.append("            $(\".youpager-pagebutton\").click(function(){\r\n");
		...
		//页面跳转
		sb.append("            $(\".youpager-btngopage\").click(function(){\r\n");
		...
		//添加页面编号
		sb.append("                   attcheForm.append('<input type=\"hidden\" name=\"currentpageindex\"  value=\"'+$(\"#youpager_txtgopage\").val()+'\"/>');\r\n");
		
		sb.append("                 }\r\n");
		//如果存在则直接赋予页号值
		sb.append("               else       ");
		
		sb.append("                   attcheForm.find(\"[name=currentpageindex]:hidden\").val( $(\"#youpager_txtgopage\").val());\r\n");
